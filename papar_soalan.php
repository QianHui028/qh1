...
idtopik=$topik_pilihan AND jenis=$jenis";
$results = mysqli_query($hubung, $query);
$total=mysqli_num_rows(results);
//TETAPKAN NOM SOALAN
$query = (int)$_GET['n'];
//DAPATAN SOALAN
$query = "SELECT*FROM soalan WHERE nom_soalan = $number
AND idtopik=$topik_pilihan AND jenis=$jenis";
//PAPAR RESULAT
$result = mysqli_query($hubung,$query);
$question= mysqli_fetch_assoc($result);
//PILIHAN JAWAPAN
$query="select*from pilihan where nom_soalan = 
$number AND idsoalan = '$question[idsoalan]'";
//PAPARKAN
...
if($numer==1){
	...
	if($jawapan=="TEPAT"){
		...
		echo $number-1;
		echo " adalah <font coor='red' size='+3'>SALAH</font>";
	}
}
?>
</td>
...
<br>
<?phpif ($question['gambarajah']==NULL){
}
else{
